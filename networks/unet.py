...
class Upsample2(nn.Module):
    def __init__(self, dim):
        super().__init__()
        self.conv = nn.Conv2d(dim, int(dim/2), 3, padding=1)
    def forward(self, x):
        return self.conv(x)
...
class UNet(nn.Module):
    def __init__(
        ...
        dropout=0,
        with_time_emb=True,
        with_feature_emb=False,
        image_size=128
    ):
        super().__init__()
        self.with_feature_emb=with_feature_emb
        if with_time_emb:
        ...
        for ind in reversed(range(num_mults)):
            ...
            for _ in range(0, res_blocks+1):
                ...
                pre_channel = channel_mult
                if self.with_feature_emb:
                    ups.append(Upsample2(pre_channel*2))
            if not is_last:
        ...
        self.sig = nn.Sigmoid()
    def forward(self, x, feature=[],time=0):
        ...
        for layer in self.ups:
            if isinstance(layer, ResnetBlocWithAttn):
                ...
                features.append(x)
            elif isinstance(layer,Upsample2):
                x = layer(torch.cat((x, feature.pop()), dim=1))
            else:
                x = layer(x)

        if self.with_feature_emb==False:
            features.reverse()
            return self.final_conv(x),features
        else:
